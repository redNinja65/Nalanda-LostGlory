...
public class ChapterSelectGUI : BaseGUI {
	...
	public override void Start() {
		...
		// get saved chapter
		currentSave = PlayerPrefs.GetInt("HighestCompletedChapters", -1);
		...
		// 0
		c.Number = currentNum;
		c.DisplayName = "Thing with Divyanshu";
		...
		// 1
		c.Number = currentNum;
		c.DisplayName = "Thing with Dipanshu";
		...
		// 2
		c.Number = currentNum;
		c.DisplayName = "Thing with Aditi";
		...
		// 3
		c.Number = currentNum;
		c.DisplayName = "Thing with Mansi";
		c.SceneName = "7-05 Credits 1";
		chapters.Add (c);
		currentNum++;
...
//
	...
	public override void OnGUI() {
		...
		GUILayout.BeginArea(new Rect(scalePx(30), scalePx(40),Screen.width+scalePx(40), Screen.height +scalePx(40)));
		...
		for(int j = 0; j < 1; j++) {
			...
			for(int i = 0 ; i < 4 ; i++) {
				...
				if(currentSave < i) { 
					GUI.enabled = false;
					GUILayout.Button("Locked Chapter " + c.Number + "\n<b>"+c.DisplayName +"</b>\n", guiSkin.customStyles[6]);
					GUI.enabled = true;
		...
		if(GUI.Button(new Rect(scalePx(300),
